...
class DiacritizationModel(ABC):
    ...
    def decode_wfst(self, logits, pattern, constrained=True):
        '''
        Decode WFST.
        Args:
        ...
        Returns:
            decoded text
        '''
        fsa = self.build_pattern_fsa(pattern, constrained=constrained)
        ...
        result = "".join(self.token2id[i] for i in hyp_ids)
        return result
    
    def decode_ctc(self, logits, pattern, constrained=True):
        '''
        Decode CTC.
        Args:
            logits: logits from the model
