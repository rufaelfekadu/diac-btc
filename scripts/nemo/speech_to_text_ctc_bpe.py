...
@hydra_runner(config_path="conf", config_name="fast-conformer_ctc_bpe")
def main(cfg):
    ...
    pretrained_model = EncDecCTCModelBPE.restore_from(cfg.init_from_nemo_model, map_location="cpu")
    pretrained_model.change_vocabulary(cfg.model.tokenizer.dir, cfg.model.tokenizer.type)
    pretrained_model.trainer = trainer
    
    if cfg.freeze_encoder:
        pretrained_model.encoder.freeze()
        pretrained_model.encoder.apply(enable_bn_se)
    ...
    if hasattr(cfg.model, 'test_ds') and cfg.model.test_ds.manifest_filepath is not None:
        if pretrained_model.prepare_test(trainer):
